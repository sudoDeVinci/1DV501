...
"""Program reads a positive odd integer from the keyboard, and then prints two triangles. First a right-angled triangle,
then an isosceles triangle."""

from math import ceil
from sys import exit

"""layers is initialized as Nonhe type so it can be compared without throwing an error."""
layers = None
...
def right_angle(layers):
    ...
    """We start fron the top layer with the most elemnts, then remove elements 
...
def isocilles(layers):
    ...
    """An isocilles triangle has half as many layers as it does elements in its largest layer.
        Therefore we iterate over a range of half our layer number
        
        We replace the elements on both ends of each layer, forward using positive incrementing of their indexes, 
        and backwards using negative incrementation of negative indexes"""
    for layer in range(1,ceil(layers/2)):
        
        """Replace the front and back element with empty space, then increment the indexes"""
    ...
    for layer in layer_list:
        """The list of layers is iterated over but is printed out in reverse order using nagative indexes."""
