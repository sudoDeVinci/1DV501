...
of steps have been taken or when a step goes outside the given boundary of the
plane.

This program assumes a grid plane with center (0,0).The size of the plane (k)
is user determined and means a plane with boundaries x,y such that
|x|<=|k|, |y|<=|k|, k>0.
"""
...
"""
Since we needs mutiple instances of essentially the same object performing
the same function(s),it stands to reason to use a class to define the
behaviour of a sailor and call the appropritate methods to make them walk
when time.
"""

# The class 'drunk' defines the behaviour of each drunk sailor.
...
class drunk:
    ...
    def __init__(self, steps):
        """
        Our number of steps left will be used during our walk function
         to check against our moves. It is initialized as the maximum
         number of steps we can make.
        """
    ...
    def walk(self, plain_size):
        # This simulates walking around our plain with given constraints
        while self.steps_left > 0:
            # For each walk loop, we move between -1 to 1 steps in our x or y
            x_steps = randint(-1, 1)
            y_steps = randint(-1, 1)

            steps_taken = abs(x_steps) + abs(y_steps)

            # If we have 1 step left, about to move 2 steps, move 1 instead
            ...
            # If we are not within our plain boundaries, set overbaord to True
...
def integer_input(input_message):
    # A simple parser to make sure our input is a positive integer
    while True:
        try:
...
def Main():
    ...
    # Create the specified number of sailors and call their walk method
    for walk in range(walks):

        sailor = drunk(steps)
        sailor.walk(plain_size)

        if sailor.overboard is True:
            overboard += 1
